...
#    textResult.insert(1.0, soup.find('div', 'listing__items').prettify())
    ...
    for number in range(car_advt_count):
        ...
        xmlFile.write('        <year>' + year.split()[0] + '</year>\n')
        xmlFile.write('        <engtype>' + engtype + '</engtype>\n')
        xmlFile.write('        <engcapacity>' + engcapacity.split()[0] + '</engcapacity>\n')
        xmlFile.write('        <fueltype>' + fueltype + '</fueltype>\n')
        xmlFile.write('        <mileage>' + ''.join(filter(str.isdigit, mileage)) + '</mileage>\n')
        xmlFile.write('        <price>' + ''.join(filter(str.isdigit, price)) + '</price>\n')
        xmlFile.write('        <priceusd>' + ''.join(filter(str.isdigit, priceusd)) + '</priceusd>\n')
